...
def train_vgg16(dataloaders, device, num_epochs=50, lr=1e-4):
    print("\n" + "="*60)
    print("VGG16 EMOTION RECOGNITION TRAINING (OPTIMIZED)")
    print("="*60)
    print(f"Device: {device}")
    print(f"Training Epochs: {num_epochs}")
    print(f"Batch Size: 8")
    print("Note: Feature layers are FROZEN to speed up CPU training.")
    print("="*60)
    ...
    # FREEZE FEATURE LAYERS for speed on CPU
    ...
    # Only optimize parameters that require gradients (the classifier)
    ...
    # Save model
    torch.save(model.state_dict(), 'best_vgg16_emotion.pth')
    print("Model saved as 'best_vgg16_emotion.pth'")
...
def build_dataloaders(root_dir, image_size=224, batch_size=8, val_split=0.2, max_samples=1000):
    ...
    # Enforce standard FER2013 mapping regardless of folder names
    # Index -> Emotion: 0:Angry, 1:Disgust, 2:Fear, 3:Happy, 4:Sad, 5:Surprise, 6:Neutral
    fer_classes = ['angry', 'disgust', 'fear', 'happy', 'sad', 'surprise', 'neutral']
    class_to_idx = {cls: i for i, cls in enumerate(fer_classes)}
    
    # Re-map existing dataset labels
    dataset.class_to_idx = class_to_idx
    dataset.samples = dataset.make_dataset(dataset.root, class_to_idx, dataset.extensions, is_valid_file=None)
...
def main():
    ...
    metrics = train_vgg16(dataloaders, device, num_epochs=25, lr=1e-4)
